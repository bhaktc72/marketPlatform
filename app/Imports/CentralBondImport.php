...
class CentralBondImport implements ToModel
{
    public function model(array $row)
    {
        return new CentralGovtBonds([
            'isin'             => $row[0],
            'nomenclature'     => $row[1],
            'dateOfIssue'      => $this->transformDate($row[2]), // Handle dateOfIssue
            'dateOfMaturity'   => $this->transformDate($row[3]), // Handle dateOfMaturity
            'outStandingStock' => $row[4],
        ]);
    }
