        ...
        <div class="card border-0 shadow mt-5">
            <div class="card-header text-white d-flex justify-content-between align-items-center" style="background-color: #EAD9F1;">
                ...
                <div>
                    ...
                    <button type="button" class="btn btn-light btn-sm d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#uploadBondModal">
                        ...
                        <tbody>
                            @foreach ($bonds as $bondsData)
                                <tr>
                                    ...
                                    {{-- <td>{{ $bondsData->id }}</td> --}}
                                    <td contenteditable="true" onblur="updateBond(this, '{{ $bondsData->id }}', 'isin')">
                                        {{ $bondsData->isin }}
                                    </td>
                                    <td contenteditable="true" onblur="updateBond(this, '{{ $bondsData->id }}', 'issuer')">
                                        {{ $bondsData->issuer }}
                                    </td>
                                    <td>{{ $bondsData->coupon_rate }}</td>
                                    <td>{{ $bondsData->maturity_date }}</td>
                                    <td>{{ $bondsData->rating }}</td>
                                    <td>{{ $bondsData->segmentOfIssuer }}</td>
                                    <td>{{ $bondsData->ModelYield }}</td>
                                    <td>{{ $bondsData->modelPrice }}</td>
                                    <td>{{ $bondsData->Y15DaysYield }}</td>
                                    <td>{{ $bondsData->P15DaysPrice }}</td>
                                    <td>{{ $bondsData->finalYield }}</td>
                                    <td>{{ $bondsData->finalPrice }}</td>
                                    <td>{{ $bondsData->remarks }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
